...
void load_config_from_string(sds config) {
	...
	for (i = 0; i < totlines; i++) {
		...
		if (strcasecmp(argv[0], "working-directory") == 0 && argc == 2) {
			...
			else if (strcasecmp(argv[1], "look-back window") == 0
					|| strcasecmp(argv[1], "lbw") == 0)
				destor.rewrite_algorithm[0] = REWRITE_LBW;
			else if (strcasecmp(argv[1], "lbw_slide_only") == 0)
				destor.rewrite_algorithm[0] = REWRITE_LBW_SLIDE_ONLY;
			else {
		...
		} else if (strcasecmp(argv[0], "rewrite-lbw-limit") == 0
				&& argc == 3) {
			destor.rewrite_lbw_drd_limit = atof(argv[1]);
			destor.rewrite_lbw_cr_limit = atoi(argv[2]);
