...
gint g_record_descmp_by_chunk_num(struct containerRecord* a,
		struct containerRecord* b, gpointer user_data) {
	return b->chunk_num - a->chunk_num;
}

gint g_record_cmp_by_chunk_num(struct containerRecord* a,
		struct containerRecord* b, gpointer user_data) {
...
void start_rewrite_phase() {
	...
	} else if (destor.rewrite_algorithm[0] == REWRITE_LBW) {
        pthread_create(&rewrite_t, NULL, lbw_rewrite, NULL);
    } else if (destor.rewrite_algorithm[0] == REWRITE_LBW_SLIDE_ONLY) {
        pthread_create(&rewrite_t, NULL, lbw_slide_only, NULL);
    }else {
