...
void destor_start() {
	...
	if ((fp = fopen(stat_file, "r"))) {
		...
		fread(&destor.last_chunk_num, 8, 1, fp);
		fread(&destor.last_unique_chunk_num, 8, 1, fp);
		...
		destor.last_chunk_num = 0;
		destor.last_unique_chunk_num = 0;
...
void destor_shutdown() {
	...
	fwrite(&destor.last_chunk_num, 8, 1, fp);
	fwrite(&destor.last_unique_chunk_num, 8, 1, fp);
