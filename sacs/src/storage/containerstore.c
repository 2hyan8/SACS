...
static int64_t container_count = 1;
static int64_t small_container_count = -3;
...
static pthread_t append_t2;


static SyncQueue* container_buffer;
static SyncQueue* small_container_buffer;
...
static void* append_thread2(void *arg) {

	while (1) {
		struct container *c = sync_queue_get_top(small_container_buffer);
		if (c == NULL)
			break;
		...
		sync_queue_pop(small_container_buffer);
...
void init_small_container_store() {

	sds small_containerfile = sdsdup(destor.working_directory);
	small_containerfile = sdscat(small_containerfile, "/small_container.pool");

	if ((fp2 = fopen(small_containerfile, "r+"))) {
		fread(&small_container_count, 8, 1, fp2);
	} else if (!(fp2 = fopen(small_containerfile, "w+"))) {
		perror(
				"Can not create small_container.pool for read and write because");
		exit(1);
	}

	sdsfree(small_containerfile);

	small_container_buffer = sync_queue_new(25);


	pthread_create(&append_t2, NULL, append_thread2, NULL);

    NOTICE("Init small_container store successfully");
}
...
void close_small_container_store() {
	sync_queue_term(small_container_buffer);

	pthread_join(append_t2, NULL);
	NOTICE("append phase stops successfully!");

	fseek(fp2, 0, SEEK_SET);
	fwrite(&small_container_count, sizeof(small_container_count), 1, fp2);
...
struct container* create_small_container() {
	struct container *c = (struct container*) malloc(sizeof(struct container));
	if (destor.simulation_level < SIMULATION_APPEND)
		c->data = calloc(1, CONTAINER_SIZE);
	else
	...
	init_container_meta(&c->meta);
	c->meta.id = small_container_count--;
	return c;
}
...
void write_container_async(struct container* c) {
	...
	if(c->meta.id > 0)
		...
		sync_queue_push(small_container_buffer, c);
...
void write_container(struct container* c) {
	...
	if (c->meta.id > 0){
		if (destor.simulation_level < SIMULATION_APPEND) {
			...
			if (fseek(fp, (c->meta.id-1) * CONTAINER_SIZE + 8, SEEK_SET) != 0) {
			...
			if(fseek(fp, (c->meta.id-1) * CONTAINER_META_SIZE + 8, SEEK_SET) != 0){
				perror("Fail seek in container store.");
				exit(1);
			}
		...
		if (destor.simulation_level < SIMULATION_APPEND) {
			...
			unsigned char * cur = &c->data[CONTAINER_SIZE - SMALL_CONTAINER_META_SIZE];
			ser_declare;
			ser_begin(cur, SMALL_CONTAINER_META_SIZE);
			ser_int64(c->meta.id);
			ser_int32(c->meta.chunk_num);
			ser_int32(c->meta.data_size);
			...
			ser_end(cur, SMALL_CONTAINER_META_SIZE);

			pthread_mutex_lock(&mutex);

			if (fseek(fp2, (c->meta.id * (-1) - 3) * CONTAINER_SIZE + 8, SEEK_SET) != 0) {
				perror("Fail seek in small_container store.");
				exit(1);
			}
			if(fwrite(c->data, CONTAINER_SIZE, 1, fp2) != 1){
				perror("Fail to write a small_container in small_container store.");
				exit(1);
			}

			pthread_mutex_unlock(&mutex);
		} else {
			char buf[SMALL_CONTAINER_META_SIZE];
			memset(buf, 0, SMALL_CONTAINER_META_SIZE);

			ser_declare;
			ser_begin(buf, SMALL_CONTAINER_META_SIZE);
			ser_int64(c->meta.id);
			ser_int32(c->meta.chunk_num);
			ser_int32(c->meta.data_size);
			...
			ser_end(buf, SMALL_CONTAINER_META_SIZE);

			pthread_mutex_lock(&mutex);

			if(fseek(fp2, (c->meta.id * (-1) - 3) * SMALL_CONTAINER_META_SIZE + 8, SEEK_SET) != 0){
				perror("Fail seek in small_container store.");
				exit(1);
			}
			if(fwrite(buf, SMALL_CONTAINER_META_SIZE, 1, fp2) != 1){
				perror("Fail to write a small_container in small_container store.");
				exit(1);
			}
...
struct container* retrieve_container_by_id(containerid id) {
	...
	if (c->meta.id > 0){
		if (destor.simulation_level >= SIMULATION_RESTORE) {
			...
			if (destor.simulation_level >= SIMULATION_APPEND)
				fseek(fp, (id-1) * CONTAINER_META_SIZE + 8, SEEK_SET);
			else
				fseek(fp, id * CONTAINER_SIZE - CONTAINER_META_SIZE + 8,
				SEEK_SET);
			...
			fseek(fp, (id-1) * CONTAINER_SIZE + 8, SEEK_SET);
		...
		if (destor.simulation_level >= SIMULATION_RESTORE) {
			c->data = malloc(SMALL_CONTAINER_META_SIZE);

			pthread_mutex_lock(&mutex);

			if (destor.simulation_level >= SIMULATION_APPEND)
				fseek(fp2, (id*(-1)-3) * SMALL_CONTAINER_META_SIZE + 8, SEEK_SET);
			else
				fseek(fp2, (id*(-1)-2) * CONTAINER_SIZE - SMALL_CONTAINER_META_SIZE + 8,
				SEEK_SET);

			fread(c->data, SMALL_CONTAINER_META_SIZE, 1, fp2);
			...
			fseek(fp2, (id*(-1)-3) * CONTAINER_SIZE + 8, SEEK_SET);
			fread(c->data, CONTAINER_SIZE, 1, fp2);

			pthread_mutex_unlock(&mutex);

			cur = &c->data[CONTAINER_SIZE - SMALL_CONTAINER_META_SIZE];
		}

		unser_declare;
		unser_begin(cur, SMALL_CONTAINER_META_SIZE);
		...
		unser_end(cur, SMALL_CONTAINER_META_SIZE);
...
struct containerMeta* retrieve_container_meta_by_id(containerid id) {
	...
	if(id > 0){
		...
		unsigned char buf[SMALL_CONTAINER_META_SIZE];
		...
		fread(buf, SMALL_CONTAINER_META_SIZE, 1, fp2);

		pthread_mutex_unlock(&mutex);

		
		unser_begin(buf, SMALL_CONTAINER_META_SIZE);
...
int container_overflow(struct container* c, int32_t size) {
	if(c->meta.id > 0){
		if (c->meta.data_size + size > CONTAINER_SIZE - CONTAINER_META_SIZE){
			printf("1--%d,  %d\n",c->meta.id, (c->meta.chunk_num + 1) * 28 + 16);
			jcr.normal_overflow++;
			return 1;
		}
		/*
		...
		*/
		if ((c->meta.chunk_num + 1) * 28 + 16 > CONTAINER_META_SIZE){
			printf("2--------%d  %d\n",c->meta.id, c->meta.data_size + size);
			jcr.small_overflow++;
			return 1;
		}
	}else{
		if (c->meta.data_size + size > CONTAINER_SIZE - SMALL_CONTAINER_META_SIZE){
			printf("1--%d,  %d\n",c->meta.id, (c->meta.chunk_num + 1) * 28 + 16);
			jcr.normal_overflow++;
			return 1;
		}
		...
		* 28 is the size of metaEntry.
		*/
		if ((c->meta.chunk_num + 1) * 28 + 16 > SMALL_CONTAINER_META_SIZE){
			printf("2--------%d  %d\n",c->meta.id, c->meta.data_size + size);
			jcr.small_overflow++;
			return 1;
		}
