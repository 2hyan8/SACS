...
#define Small_Chunk_define 1024
...
struct{
    ...
    struct container *small_container_buffer;
...
static void* filter_thread(void *arg) {
        ...
        for (; iter != end; iter = g_sequence_iter_next(iter)) {
            ...
            if (!CHECK_CHUNK(c, CHUNK_DUPLICATE) 
                ...
                if(c->size > Small_Chunk_define){
                    ...
                    if (storage_buffer.small_container_buffer == NULL){
                        storage_buffer.small_container_buffer = create_small_container();
                        if(destor.index_category[1] == INDEX_CATEGORY_PHYSICAL_LOCALITY)
                            storage_buffer.chunks_2 = g_sequence_new(free_chunk);
                    }

                    if (container_overflow(storage_buffer.small_container_buffer, c->size)) {
                        
                        if(destor.index_category[1] == INDEX_CATEGORY_PHYSICAL_LOCALITY){
                            
                            GHashTable *features = sampling(storage_buffer.chunks_2,
                                    g_sequence_get_length(storage_buffer.chunks_2));
                            index_update(features, get_container_id(storage_buffer.small_container_buffer));
                            g_hash_table_destroy(features);
                            g_sequence_free(storage_buffer.chunks_2);
                            storage_buffer.chunks_2 = g_sequence_new(free_chunk);
                        }
                        TIMER_END(1, jcr.filter_time);
                        write_container_async(storage_buffer.small_container_buffer);
                        TIMER_BEGIN(1);
                        storage_buffer.small_container_buffer = create_small_container();
                    }

                    if(add_chunk_to_container(storage_buffer.small_container_buffer, c)){
                        ...
                        if(destor.index_category[1] == INDEX_CATEGORY_PHYSICAL_LOCALITY){
                            struct chunk* ck = new_chunk(0);
                            memcpy(&ck->fp, &c->fp, sizeof(fingerprint));
                            g_sequence_append(storage_buffer.chunks_2, ck);
                        }
    ...
    if (storage_buffer.small_container_buffer
    		&& !container_empty(storage_buffer.small_container_buffer)){
        if(destor.index_category[1] == INDEX_CATEGORY_PHYSICAL_LOCALITY){
            /*
             * TO-DO
             * Update_index for physical locality
